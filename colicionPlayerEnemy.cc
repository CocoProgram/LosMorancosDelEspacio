...
bool CollisionPlayerEnemy(TPlayer Player) {
  ...
  for(int i=0;i<g_num_enemy;i++){
		...
		if(tmp.is_alive){
			...
			&& (Player.posx + esat::SpriteWidth((*(Player.player_sprites)))) < (tmp.posx + (esat::SpriteWidth((*(enemy.enemy_sprites))))))
			&&  Player.posy 											     > tmp.posy
			&&  Player.posy											         < (tmp.posy + (esat::SpriteHeight((*(enemy.enemy_sprites))))))		
			){	
				printf("Player X= % 3f -- Player Y= % 3f -- Enemy X= % 3f -- Enemy Y= % 3f\n", Player.posx, Player.posy, tmp.posx, tmp.posy);
			...
			&& (Player.posy + esat::SpriteHeight((*(Player.player_sprites)))) < (tmp.posy + (esat::SpriteHeight((*(enemy.enemy_sprites)))))		
			){
				printf("Player X= % 3f -- Player Y= % 3f -- Enemy X= % 3f -- Enemy Y= % 3f\n", Player.posx, Player.posy, tmp.posx, tmp.posy);
				return true;
			}
			...
			&& (Player.posy + (esat::SpriteHeight((*(Player.player_sprites)))/3)) < (tmp.posy + (esat::SpriteHeight((*(enemy.enemy_sprites)))))		
			){	
				//printf("Colicion_1");
				return true;
			}
			...
			&& (Player.posy + (((esat::SpriteHeight((*(Player.player_sprites))))/3)*2)) < (tmp.posy + (esat::SpriteHeight((*(enemy.enemy_sprites)))))		
			){	
				//printf("Colicion_2");
				return true;
			}
